...
import { addPrepareSendFiles } from "@/services/CacheService";
import { AlertError } from "@/utils/core";

export default function DropdownAttach() {
  ...
  function onImageChange(event: any) {
    ...
    setShowDropdownAttach(!showDropdownAttach)
    if (event.target.files && event.target.files.length > 0) {
      for (const file of event.target.files) {
        ...
        if(appState.prepareSendFiles.length > 0 && appState.prepareSendFiles.find((file) => file.size === file.size)) return
