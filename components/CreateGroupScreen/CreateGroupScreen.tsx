    ...
    friendSnapshot?.docs?.forEach((friend) => {
      (async () => {
        const userSnapshot = await db
          .collection("users")
          .where("email", "==", getRecipientEmail(friend.data().users, user))
          .get();
    ...
    if (friendList.length > 0) {
        ...
        return friendList
          ? friendList?.map((data: any) => (
              <ItemFilter
                key={data.id}
                userInfo={data}
                onCheckBox={(userInfo: any, checked: boolean) =>
                  handleCheckBox(userInfo, checked)
                }
                checked={listMember?.find((mem: any) => mem.id === data.userId)}
              />
            ))
          : null;
      } else {
      ...
      if (searchInput.length === 0) {
        return friendSnapshot
          ? friendSnapshot?.docs?.map((data: any) => (
              <Member
                key={data.id}
                email={getRecipientEmail(data.data().users, user)}
                onCheckBox={(userInfo: any, checked: boolean) =>
                  handleCheckBox(userInfo, checked)
                }
      ...
      <Container>
        <SidebarContainer>
          <Search className="py-2">
            <SearchIcon />
            <SearchInput
              placeholder="Enter phone number or email to search"
              value={searchInput}
          ...
          <BtnContainer className="py-2 text-center">
            <Button
              ...
              style={{
                display:
                  listMember?.length! >= 2 && groupName.length >= 3
                    ? "block"
                    : "none",
              }}
